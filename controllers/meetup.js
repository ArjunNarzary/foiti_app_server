...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    if (place && place.place_id != "") {
      ...
      if (user.place) {
        if (
          user.place.google_place_id.toString() !== place.place_id.toString()
        ) {
          ...
          //DELETE PLACE
          if (
            prevPlace.posts.length === 0 &&
            prevPlace.users.length === 0 &&
            !prevPlace.reviewed_status
          ) {
            ...
            if (reviews.length > 0) {
              ...
              reviews.forEach(async (reviewData) => {
                ...
                if (
                  userContribution.review_200_characters.includes(
                    reviewData._id
                  )
                ) {
    ...
    await TripPlan.updateMany(
      { $and: [{ user_id: authUser._id }, { meetup_status: false }] },
      { $set: { meetup_status: true } }
    )
...
exports.getLocals = async (req, res) => {
  let errors = {}
  try {
    let { skip, address, placesArr } = req.body
    const limit = 10
    let noMoreData = false
    ...
    const locals = await User.find({ place: { $in: placesArr } })
      ...
      .ne(true)
      .sort({ meetup_points: -1, total_contribution: -1, _id: -1 })
      .select(
        "_id name profileImage gender dob bio meetup_reason interests education occupation languages movies_books_music"
      )
      .populate(
        "place",
        "name display_name address display_address_available display_address display_address_for_own_country display_address_for_other_country original_place_id"
      )
      .skip(skip)
      .limit(limit)
    ...
    locals.forEach((user) => {
      ...
      user.place.local_address = user.place.display_address_for_own_country_meetup_list
...
//Version 10
exports.getLocalsv10 = async (req, res) => {
  let errors = {}
  try {
    let { skip, address, placesArr } = req.body
    const limit = 10
    ...
    if (placesArr.length === 0) {
      ...
      let place;
      if (address?.place_id) {
        place = await Place.findOne().where('google_place_id').equals(address?.place_id)
      }

      if(place){
        if (place.types[1] === "country") {
          await Place.find({ users: { $exists: true, $ne: [] } })
  ...
  try {
    ...
    if (ip) {
      ...
      if (user.place?._id) {
        if (user.place.address.short_country == country) {
            user.place.local_address =
                user.place.display_address_for_own_country_meetup_profile
        } else {
            user.place.short_address =
                user.place.display_address_for_other_country_meetup_profile
        }
    }
    }else{
        user.place.local_address =
...
//Version 8
exports.getLocalDetails = async (req, res) => {
  const errors = {}
  try {
...
exports.requestMeetup = async (req, res) => {
  ...
  try {
    ...
    if (meetupChat) {
      ...
      const newChat = await MeetupChat.findOne({
        _id: createdChat._id,
      }).populate("chatUsers", "name email profileImage blocked_users")
